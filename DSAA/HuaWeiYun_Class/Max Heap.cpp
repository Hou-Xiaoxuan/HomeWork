...
bool MaxHeap<T>::Remove(int pos, T& node)
{
    ...
    CurrentSize--;
    node=heapArray[pos];
    swap(heapArray[pos],heapArray[CurrentSize]);
    if(heapArray[pos]>heapArray[CurrentSize])
    {
        heapArray[CurrentSize]=-1;
        SiftUp(pos);
    }
    else{
        heapArray[CurrentSize]=-1;
...
void MaxHeap<T>::SiftDown(int left)
{
    ...
    int x=leftchild(left);
    if(heapArray[x+1]>heapArray[x]) x++;
    if(x<CurrentSize and heapArray[x]>heapArray[left]){
        swap(heapArray[x],heapArray[left]);
        SiftDown(x);
    }
...
{
    if(pos>=CurrentSize) return;
    if(pos==0) return;
    int fa=parent(pos);
    if(heapArray[fa]<heapArray[pos]){
        swap(heapArray[fa],heapArray[pos]);
        SiftUp(fa);
    }
}
